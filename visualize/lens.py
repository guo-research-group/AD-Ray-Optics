...
def visualize_lens(sm, N = 100 , radius = 2):
    ...
    for surf_num in range(1, sm.get_num_surfaces()-1):
      ...
      data.append(go.Mesh3d(x=x, y=z, z=y, i=i, j=j, k=k, intensity=x, colorscale='armyrose', opacity=1))
      ...
      if (lens):
        ...
        # The indices of the triangles are stored in the `simplices` attribute
        i, j, k = tri.simplices.T
        data.append(go.Mesh3d(x=x, y=z, z=y, i=i, j=j, k=k, colorscale='ice', opacity=0.5))
        data.append(go.Mesh3d(x=x, y=z, z=-y, i=i, j=j, k=k, colorscale='ice', opacity=0.5))
        lens = False
