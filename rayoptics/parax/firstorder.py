...
"""
#import math
from jax.numpy import radians, tan, inf, sin, degrees
from jax.lax import asin, atan
...
def paraxial_trace(path, start, start_yu, start_yu_bar):
    ...
    b4_rndx *= np.sign(z_dir_before) #modified
    n_before = b4_rndx #changed
    #b4_rndx *= np.sign(z_dir_before) #changed
    ...
    while True:
        try:
            ...
            if ifc.interact_mode == 'dummy':
                ...
                rndx *= np.sign(z_dir_after)#changed
                n_after = rndx #changed
...
def compute_first_order(opt_model, stop, wvl):
    ...
    if stop:
        ...
        #if max_fld == 0.0:
            #max_fld = 1.0
        max_fld = (max_fld == 0) + (max_fld != 0) * max_fld     
        if obj_img_key == 'object':
            if value_key == 'angle':
                ang = radians(max_fld)
                slpbar0 = tan(ang)
    ...
    fod.obj_ang = degrees(atan(pr_ray[0][slp]))
    if pr_ray[0][slp] != 0:
    ...
    fod.img_na = n_k*sin(atan(seq_model.z_dir[-1]*ax_ray[-1][slp]))
