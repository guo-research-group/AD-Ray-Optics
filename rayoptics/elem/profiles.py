...
from jax import lax
from rayoptics.util.misc_math import normalize, condition_replacement
from rayoptics.raytr.traceerror import TraceError, TraceMissedSurfaceError

from . import myFunctions as my
...
class SurfaceProfile:
    ...
    def intersect_spencer(self, p0, d, eps, z_dir):
        ...
        q= my.special_dot(d, self.df(p)) #changed
        s1 = -self.f(p)/my.special_dot(d, self.df(p)) #changed
        delta = abs(s1)
        # print("intersect", s1)
        iter = 3
        # while delta > eps and iter < 1000:          #changed the number of iterations
        for iter in range(3):
            #p = p0 + s1*d
            p = my.special_p(p0,s1,d)
            #s2 = s1 - self.f(p)/np.dot(d, self.df(p))
            s2 = s1 - self.f(p)/my.special_dot(d, self.df(p))  #changed
            delta = abs(s2 - s1)
        ...
        # print('intersect iter =', iter)
        return s1, p
        #lax.fori_loop(0,1,p,p0)
        #cond_func1 = lambda delta: delta>eps
        #cond_func2 = lambda iter: iter<4
        #body_func1 = lambda p : my.special_p(p0,s1,d)
        #body_func2 = lambda s2 : s1 - self.f(p)/my.special_dot(d, self.df(p))
        #body_func3 = lambda delta: abs(s2-s1)
        #body_func4 = lambda s1:s2
        #lax.while_loop(cond_func2, body_func1, p0)
        #lax.while_loop(cond_func2, body_func2, 0)
        #lax.while_loop(cond_func2, body_func3, eps)
        #lax.while_loop(cond_func2, body_func4, s1)
    ...
    def intersect_scipy(self, p0, d, eps, z_dir):
        ...
        s = sol.root
        #p1 = p0 + s*d
        p1 = my.special_p(p0,s,d) #changed
...
#        return super().intersect(p, d, eps, z_dir)
        ...
        cx2 = self.cv * my.special_dot(p,p) - 2*p[2] #changed
        b = self.cv * my.special_dot(d, p) - d[2]
        try:
            # Use z_dir to pick correct root
            s = cx2/(z_dir*sqrt(b*b - ax2*cx2) - b)
        except ValueError:
    ...
    def f(self, p):
        return p[2] - 0.5*self.cv*(my.special_dot(p, p)) #changed
    ...
    def profile(self, sd, dir=1, steps=6):
        ...
        if self.cv != 0.0:
            ...
            total_cos = my.special_dot(dir1, dir2) #changed
...
#    @coef.setter
    ...
    def gen_coef_list(self):
        if len(self.coefs) == 0:
            ...
            self.coefs.append(self.coef10)
            self.max_nonzero_coef = -1
        self.max_nonzero_coef = len(self.coefs)
        # for i, c in enumerate(self.coefs):
        #     #if c != 0.0:  
        #         #self.max_nonzero_coef = i
        #     # c=[c!=0]    #changed
        #     self.max_nonzero_coef = condition_replacement(c==0, i, self.max_nonzero_coef)
        #     # self.max_nonzero_coef = i    #changed
        # self.max_nonzero_coef += 1 
        # self.max_nonzero_coef =  # brute force
    ...
    def df(self, p):
        # sphere + conic contribution
        r2 = p[0]*p[0] + p[1]*p[1]+1e-18 #modified 
        r2prev = p[0]*p[0] + p[1]*p[1]
        #print(r2prev)
        #print(r2)
        r = sqrt(r2)
        rprev = sqrt(r2prev)
        ...
        #r = stop_gradient(r) # observation: dr/dp causing problem
        #r = 1
        r = condition_replacement(r==0.0, 1.0, r)
        r_pow = 1/r
        #r_pow = (1-r)*(1/r)
        # if r == 0.0:
        #     r_pow = 1.0/1
